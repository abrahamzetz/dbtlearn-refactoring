...
FROM analytics.raw.orders as Orders
left join (select ORDERID as order_id, max(CREATED) as payment_finalized_date, sum(AMOUNT) / 100.0 as total_amount_paid
        from analytics.raw.payment
        where STATUS <> 'fail'
        group by 1) p ON orders.ID = p.order_id
left join analytics.raw.customers C on orders.USER_ID = C.ID ),
...
left join analytics.raw.orders as Orders
